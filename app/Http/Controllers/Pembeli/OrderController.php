...
class OrderController extends Controller
{
    ...
    public function callback(Request $request)
    {
        try {
            ...
            $processed = $this->updateOrderStatus($request->order_id, $request->transaction_status);
    ...
    // Sinkronkan status order(a) berdasarkan status transaksi dari Midtrans
    protected function updateOrderStatus($orderIdMidtrans, $midtransStatus)
    {
        $statusBaru = $this->mapTransactionStatus($midtransStatus);

        return DB::transaction(function () use ($orderIdMidtrans, $statusBaru) {
            $orders = Order::where('order_id_midtrans', $orderIdMidtrans)
                ->lockForUpdate()
                ->get();
    ...
    // Cek & sinkronkan status pembayaran langsung ke Midtrans (dipakai front-end setelah Snap selesai)
    public function cekStatus($order_id_midtrans)
    {
        try {
            $orders = Order::where('order_id_midtrans', $order_id_midtrans)->get();

            if ($orders->isEmpty() || $orders->first()->user_id !== Auth::id()) {
                return response()->json(['success' => false, 'message' => 'Pesanan tidak ditemukan'], 404);
            }

            $result = Transaction::status($order_id_midtrans);
            ...
            if (! $transactionStatus) {
                Log::warning('Midtrans Status Check: transaction_status kosong', $result);

                return response()->json(['success' => false, 'message' => 'Status transaksi tidak diketahui'], 422);
            }

            $this->updateOrderStatus($order_id_midtrans, $transactionStatus);
